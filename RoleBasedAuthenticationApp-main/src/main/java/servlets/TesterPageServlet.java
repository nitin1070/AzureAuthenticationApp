...
public class TesterPageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
    	...
    	HttpSession session = request.getSession(false);
 	    ...
 	    if (session != null && isUserAuthenticated(session) && isAuthorized(session, "Member")) {
 	    	 String user=  (String) session.getAttribute("user");	
 			 
 	    	 session.setAttribute("user",user);
 	     

 	        request.getRequestDispatcher("/WEB-INF/TesterPages/TesterHomePage.jsp").forward(request, response);
